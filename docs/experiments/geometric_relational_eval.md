...
## 13. 再挑戦アイデア（未実装・要計測のメモ）

Commit 1〜5 の結果は EXPERIMENTAL（中立）。**実装予定ではなく、将来の検討用にアイデアだけ
残す。** 着手時はいずれも既定オフ・人間対局なし・固定時間勝率（§9 採用基準）でのみ採否を決める。

### 13.0 なぜ Stage 1 が中立だったか（診断）

- **`occ_*` は既存ライン評価と二重計上**: 角・中心が 7 本のラインに属する効果は `open1/2/3`
  が既に内包。冗長で寄与が薄い。
- **`playable_*` が粗すぎた**: 仮説の核心「位置 × 周辺関係（将棋の駒関係）」に対し、実装は
  *セル種類を数えただけ*。「そのセルが生きたラインを跨ぐか／フォークを作るか」という
  **関係情報を落としている**（死んだラインしか通らない高価値種類セルも同じく加点していた）。
- **線形・加算が弱い**: 調整済みパリティ評価へ線形項を足す形では支配的なライン/パリティ項に
  埋もれる。脅威枝刈りで中盤は終端まで解けるため、静的評価が効く帯域も狭い。

→ **ユーザー仮説の本丸（関係性・支持順序）は未実測**。再挑戦は「線形 occ/playable の微調整」
ではなく、関係情報を表現し、**目的を勝率に揃えた最適化**で測るべき。

### 13.1 案A: Stage A 拡張 — 関係特徴（仮説に最も忠実）

着地点を「セル種類 × その手で進む own/opp の生きたライン（open2→open3・フォーク交点）」で
表す。例: `playable_interior_own_open2` / `playable_corner_opp_open2`。
- 効きうる理由: 価値の高い種類のセルで実際に複数の生きたラインが交差する局面だけ加点でき、
  Stage 1 が落とした関係情報を回復する。
- 注意: 即勝ち・強制受けは探索が扱うので加点しない（非即時の potential のみ）。own/opp を分離。
- コスト: 中（特徴抽出を Python/Rust に追加＋契約テスト）。本書 §8 の延長。

### 13.2 案B: Stage B — 柱 N-tuple パターン表 `[3][31]`（支持順序を直接表現）

`COLUMN_CLASS`（Commit 1 で実装済み）を使い、柱内の占有順序（CORNER→EDGE→EDGE→CORNER 等）を
パターン表で評価する。線形 occ/playable では**原理的に表現できない**支持関係＝仮説の深層
（将棋の三駒関係 analog）。本書 §9。学習は §13.3 とセットで行う。

### 13.3 案C: 最適化を「勝率」に揃える（A/B の前提）

単独特徴スクリーニングは組合せ効果を見逃し、Phase 8 で「教師スコア回帰＝fit≠strength」も判明。
よって:
- **SPSA / 座標探索 / CMA-ES** で重みを **固定時間自己対局の勝率**を目的に直接最適化、または
- **勝敗（game outcome）のロジスティック回帰**で `default_eval` 固定の残差だけ学習
  （`learn.py` を流用可能）。
大重みが一様悪化＝最適は小さく相互作用的な重み。単独掃引では見つからない。

### 13.4 案D: 安価な事前診断（投資の前に）

A/B に進む前に、Stage 2（`opening_plies` 4/8/12）で**序盤局所のシグナルがあるか**だけ先に見る。
完全に無ければ線形方向は死んでいると確定でき無駄を避けられる。固定時間（50/100/200ms）でも
一度確認（Web 用途・NPS 込みの判断）。

### 推奨順序と見立て

**D で序盤シグナルを安く確認 → あれば A（関係特徴）を C（勝率/勝敗教師）で最適化 → A が中立なら
B（柱パターン表）で支持順序を直接テスト。**

正直な見立て: エンジンは既に強く脅威枝刈りで中盤が解けるため静的評価の伸びしろ自体が小さく、
B/C まで行っても中立で終わる可能性は十分ある。それでも仮説の本丸（関係性・順序）は未検証なので、
試す価値があるのは A / B。採用は常に固定時間勝率で判断し、中立なら既定オフのまま資産保持する。
