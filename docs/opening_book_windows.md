...
- 生成中は `out_path` と `out_path.ckpt.json` に**周期的に原子保存**される（書き込み中に落ちても
  ファイルは壊れない）。`both` の間は `opening_book.own0.json` / `opening_book.own1.json` の
  中間ファイルもできる（完走時に自動削除）。
- **`Ctrl + C` で安全に中断できる。** 直近のチェックポイント以降の探索だけがやり直しになる。
- **同じコマンドをもう一度実行すると、チェックポイントから続きを生成する。** 完走すると `.ckpt.json`
  は自動削除される。
- 長時間回すときは、Windows の電源設定でスリープ/休止を無効化し、ターミナルを閉じないこと。

### 追加編集・延長

- **より深く / より広く**したいときは、`max_plies` や `opp_width` を上げて同じ `out_path` に
  生成し直す（深い `depth` のエントリが優先される＝品質単調）。
- 既存ファイルと別生成を統合したいときは Python から `merge_book(a, b)` を使う（深い `depth` 優先）。
