...
# 定石（生成 → 照会）。選択的・途中保存・再開・追加延長・フェーズ別・評価打ち切り対応
#   中断後は同コマンド再実行で続行。--profile でフェーズ別、--cutoff で評価打ち切り
# Windows(CPU) での環境構築〜生成は docs/opening_book_windows.md を参照
PYTHONPATH=src python scripts/generate_book.py data/opening_book.json --plies 6 --depth 10 --opp-width 4 --owner both
PYTHONPATH=src python -c "
from score_four.board import Board
